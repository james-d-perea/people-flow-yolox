...
from .yolo_detection import detect_people_yolo
from .zones import filter_ignored_zones, update_ignored_zones


HEURISTIC_MODE = "Heuristic CV"
YOLO_MODE = "YOLO"
DETECTION_MODES = {HEURISTIC_MODE, YOLO_MODE}
DEFAULT_DETECTION_MODE = YOLO_MODE


def normalize_detection_mode(detection_mode: str | None) -> str:
    if detection_mode in DETECTION_MODES:
        return detection_mode
    return DEFAULT_DETECTION_MODE


def analyze_video(video_path: str | Path, detection_mode: str = DEFAULT_DETECTION_MODE) -> dict[str, Any]:
    final_update = None
    for update in analyze_video_stream(video_path, detection_mode=detection_mode):
        final_update = update
...
def analyze_video_stream(video_path: str | Path, detection_mode: str = DEFAULT_DETECTION_MODE) -> Iterator[dict[str, Any]]:
    ...
    try:
        while True:
            ...
            # We still compute the mask in YOLO mode so the UI can show the full
            # required CV stages: enhanced frame, segmentation mask, and cleaned mask.
            raw_mask = segmenter.apply(enhanced)
            cleaned = clean_mask(raw_mask)

            if detection_mode == YOLO_MODE:
                detections = detect_people_yolo(enhanced)
            else:
            ...
            if detection_mode == HEURISTIC_MODE:
            ...
            cv2.putText(
                overlay,
                f"detector: {detection_mode}",
                (10, min(56, overlay.shape[0] - 10)),
    ...
    metrics = {
        ...
        "cpu_only": True,
        "detector": (
            "Motion-driven candidates"
            if detection_mode == HEURISTIC_MODE
            else "YOLOX-Nano (Apache-2.0, COCO-pretrained) person detector"
        ),
